...
#'xvfb', 'xvnc' or 'xephyr',
    disp = SmartDisplay(visible=visible, backend=backend, size=screen_size, bgcolor=bgcolor)
...
if __name__ == '__main__':
    cmd = ['gtkwave',
           # "/home/tao/des/desmod/docs/examples/DP_allocation/workspace/sim_dp.vcd",
           # '--tcl_init',
           # tclfile.name,
           # '--rcfile',
           # rcfile.name,
           '-a',
           # '/home/tao/des/desmod/docs/examples/DP_allocation/workspace/sim.gtkw',
           '/home/tao/des/sphinxcontrib-gtkwave/tests/sim_zoom_half_fit.gtkw',
           '--nomenu',
           ]

    f = 'gtkwave_id%s.png' % (str(image_id))
    fabs = "/home/tao/des/sphinxcontrib-gtkwave/tests/gtkwave_id.png"
    screen = '1920x1080'
    screen = tuple(map(int, screen.split('x')))
    wait = 0
    timeout = 60
    ...
    # 'xvfb', 'xvnc' or 'xephyr',
    prog_shot(cmd, fabs, screen_size=screen, wait=wait,
              timeout=timeout, bgcolor=bgcolor,backend='xvnc', visible=None)
